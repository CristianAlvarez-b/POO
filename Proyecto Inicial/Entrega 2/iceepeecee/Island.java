...
public class Island
{
    private int[][] vertexMatrix;       // Coordinates of the island's vertices
    private PolygonShape polygon;       // PolygonShape representing the island
    private ArrayList<PolygonShape> border; // Borders drawn around the island
    /**
     * Create an island.
     * 
     ...
     */
    public Island(int[][] matrix, String color){
    ...
    public void drawBorder(){
        ...
        for (int i = 0; i < numVertices; i++) {
            int x1 = points[0][i];
            int y1 = points[1][i];
            int x2 = points[0][(i + 1) % numVertices]; // El módulo asegura que el último punto se conecte con el primero
            int y2 = points[1][(i + 1) % numVertices];
            int[] xPoints = {x1, x2};
            int[] yPoints = {y1, y2};
